...
export const generatePresignedPost = async (
  ...
  let localstackUrl = url.replace('https://s3.amazonaws.com', s3Endpoint).replace('https://dragdrop-documents.s3.amazonaws.com', s3Endpoint);
  
  // Replace 'localstack' hostname with 'localhost' for browser access
  localstackUrl = localstackUrl.replace('http://localstack:', 'http://localhost:');
  
  console.log('✅ [S3 SERVICE] Presigned POST URL generated (expires in 1 hour)');
...
export const getDownloadUrl = async (key: string): Promise<string> => {
  ...
  console.log('✅ [S3 SERVICE] Presigned URL generated (expires in 15 minutes)');
  return url;
};
